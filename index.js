...
(function () {
   ...
   * @returns {Object} 键值对，缺失的标签值为 null
   */
   ...
   * 解析多组重复 XML 标签（如多条帖子）
   * @param {string} raw - 原始文本
   * @param {string} wrapperTag - 外层标签名（如 'post'）
   * @param {string[]} innerTags - 每组内的标签名数组
  ...
  const Parser = {
    /**
     * 从所有聊天消息中提取 <meow_FM> 数据
     * @returns {Object[]} 每期连线数据
     */
    getAllMeowFM() {
    ...
    getLatestThinking() {
      ...
      for (let i = msgs.length - 1; i >= 0; i--) {
        ...
        // 其次 extra.reasoning（Claude API原生）
  ...
  const Notify = {
    ...
    add(icon, title, message, appId) {
      ...
      //灵动岛展示
      EventBus.emit('dynamic-island:show', { icon, title, body: message });
    },

    /**
     * 记录错误
     * @param {string} source - 错误来源（如 App名称）
  ...
  const PhoneShell = {
    ...
    buildDOM() {
      ...
      this._floatEl.innerHTML = `
        📡
        <span class="freq-float-badge"></span>
      `;
        ...
        <div id="freq-phone">
          ...
          <div id="freq-screen">
            ...
            <div id="freq-notification-drawer">
              ...
              <div class="freq-notif-list" id="freq-notif-list"><div class="freq-notif-empty">暂无通知</div>
              </div>
            </div>
    ...
    _bindEvents() {
      //悬浮球点击 → 打开手机
      this._floatEl.addEventListener('click', () => this.openPhone());
      ...
      try {
        ...
        this._appContentEl.innerHTML = `
          <div class="freq-error">
            ...
            <button class="freq-btn freq-btn-sm" onclick="this.closest('#freq-app-content').innerHTML=''">关闭</button>
          </div>
        `;
  ...
  const SettingsUI = {
    ...
    /**
     * 获取自定义 Prompt（如果用户填了就用用户的，否则返回 null）
     */
...
<message>消息内容</message>`;
        ...
        PhoneShell._updateBadge(randomApp.id, randomApp._badge);} catch (e) {
        Notify.error('后台触发', e);
      }
    },
  };
  ...
  function freqInit() {
    ...
    // 如果插件未启用，隐藏一切
    if (!settings.enabled) {
      PhoneShell.setFloatVisible(false);
    }
    ...
    try {
      ...
      if (ctx && ctx.eventSource) {
        ...
        if (ctx.eventSource.on) {
          ...
          ctx.eventSource.on(EVENT_MESSAGE_SENT, () => {
